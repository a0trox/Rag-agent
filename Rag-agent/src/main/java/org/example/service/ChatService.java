...
public class ChatService {
    ...
    @Value("${rag.model}") // 确保这对应你 yml 里的配置
    ...
    public String buildSystemPrompt(List<Map<String, String>> history) {
        ...
        // 基础系统提示
        systemPromptBuilder.append("你是一个拥有实时扩展能力的智能助手。你的核心规则如下：\n");
        systemPromptBuilder.append("1. **强制检索原则**：当用户询问任何关于内部文档、活动安排、XX计划、技术指南或公司流程的问题时，你必须先调用 `queryInternalDocs` 工具，严禁凭空想象。\n");
        systemPromptBuilder.append("2. **时间敏感性**：如果问题涉及“明天”、“下周”等时间词，先调用 `getCurrentDateTime` 获取当前时间，再根据日期去检索文档。\n");
        systemPromptBuilder.append("3. **结果整合**：如果检索不到内容，请明确告知用户‘在内部知识库中未找到相关记录’，不要说‘我无法查看文件’。\n\n");
    ...
    public String getAnswer(ReactAgent agent, String question,String userId) throws GraphRunnerException{
        List<Float> currentVector = embeddingService.generateEmbedding(question);

        // Step 1: 尝试从 Session 缓存命中
        String cachedAnswer = cachehitservice.getAnswerFromCache(userId, currentVector);
        if (cachedAnswer != null) return cachedAnswer;

        logger.info("执行 ReactAgent.call() - 自动处理工具调用");
        String answer = agent.call(question).getText();
        logger.info("ReactAgent 对话完成，答案长度: {}", answer.length());
        // Step 3: 将新结果存入 Session 缓存
        cachehitservice.saveToSessionCache(userId, question, currentVector, answer);
